...
> 📘【note】<br>
> この記事は[Kubernetes2 Advent Calendar 2020](https://qiita.com/advent-calendar/2020/kubernetes2)の18日目です。<br>
...
考察 - 運用環境への適用に向けて
---
ここまでの実験で、preStopスリープ、Graceful Shutdonwのどちらもローリングアップデート時のエラーの抑制に効果があることが分かりました。それでは、実際の運用の場面ではどちらの対策を採用していくべきなのでしょうか？

これは、実際のKubernetesの環境や、アプリケーションの特性により判断していくしかありません。

### preStopスリープの長さについて
preStopスリープの時間に関しては、endpoints-controller、kube-proxyによるサービスアウト処理が、どのくらいの速さで完了するかを考慮する必要があります。
サービスアウトにかかる時間は、クラスター内のServiceリソース数や配下のPod数、クラスターのNode数などによって変わってくると思われます。

実際の環境でサービスアウトにかかる時間を把握した上で、これを超えるようにpreStopスリープを設定してください。

### Graceful Shutdownの要否について
Graceful Shutdownの要否はアプリケーションの特性によって判断していく必要があります。
例えば、処理中のリクエストが中断されることによってデータ不整合などの重大な問題が起きうるアプリケーションでは、しっかりとGraceful Shutdownする必要があるでしょう。

また、preStopスリープを十分長く取って、処理中のリクエストさえなくなるまで待った上でシャットダウンに入る、という考え方もできそうですが、これも確実ではありません。
外部サービスやDBに依存するようなアプリケーションでは、リクエストの処理にかかる時間が想定以上となることは往々にして起こりえるのではないでしょうか。

処理を中断しても問題ないように実装できていることが明らかであったり、なるべく早くシャットダウンしたい特別な事情があるという場合に限り、Graceful Shutdownを行わない選択をとると考えるのが無難ではないでしょうか。
